...
export function Sidebar({ shared, tab }: Props) {
  ...
  return (
    <div style={{ padding: 20, background: "rgba(0,0,0,0.2)", overflowY: "auto" }}>
      {/* Live summary */}
      <div style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)", letterSpacing: "0.15em", marginBottom: 14, fontWeight: 700 }}>STRATEGY SUMMARY</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 1, marginBottom: 18 }}>
        {[
          ...
          { label: "R:R", val: `1 : ${rrRatio.toFixed(1)}`, color: rrRatio >= 2 ? "var(--accent)" : "var(--warning)" },
        ].map(row => (
          <div key={row.label} style={{ display: "flex", justifyContent: "space-between", padding: "8px 12px", background: "rgba(255,255,255,0.02)", gap: 8 }}>
            <span style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)", flexShrink: 0 }}>{row.label}</span>
            <span style={{
              fontSize: "var(--text-sm)", color: (row as any).color || "var(--text-primary)",
              textAlign: "right", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
              maxWidth: 160, fontFamily: (row as any).mono ? "var(--font-mono)" : "inherit",
              fontWeight: 500,
            }}>{row.val}</span>
          </div>
        ))}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 16 }}>
            {shared.indicators.map((ind, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "5px 8px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <span style={{ fontSize: 9, color: "#2a2a3a" }}>{ind.type}({ind.period})</span>
              </div>
            ))}
          </div>
        </>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {tips.map((tip, i) => (
          <div key={i} style={{ display: "flex", gap: 10, padding: 10, background: "rgba(255,255,255,0.015)", border: "1px solid var(--border-subtle)" }}>
            <span style={{ color: "var(--accent)", fontSize: "var(--text-sm)", flexShrink: 0 }}>◈</span>
            <span style={{ fontSize: "var(--text-xs)", color: "var(--text-secondary)", lineHeight: 1.6 }}>{tip}</span>
          </div>
        ))}
      </div>
        ...
        <>
          ...
          {SYNTAX_EXAMPLES.map(([label, ex]) => (
            <div key={label} style={{ marginBottom: 7 }}>
              <div style={{ fontSize: 9, color: "#2a2a3a", marginBottom: 2 }}>{label}</div>
